...
export default function AboutContent() {
  ...
  return (
    <div className="min-h-screen" style={{ paddingTop: 80, paddingBottom: 64, backgroundColor: '#ffffff' }}>
      ...
      {/* How Tomás Works — Six Disciplines */}
      <section style={{ backgroundColor: '#093f4f', paddingTop: 64, paddingBottom: 64 }}>
        <div className="section-container">
          <div style={{ marginBottom: 40 }}>
            <span className="label-caps block mb-4" style={{ color: '#789ead' }}>How Tomás Works</span>
            <h2
              style={{
                fontFamily: 'Georgia, "Times New Roman", serif',
            ...
            <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.5)', lineHeight: 1.75, margin: 0 }}>
              Every buyer goes through the same six disciplines:
            </p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { num: '01', title: 'Location Fit', body: 'West Bay, select West End, Sandy Bay waterfront, Pristine Bay, and emerging nodes with genuine access, views, or resort logic — matched to your lifestyle and use case.' },
              { num: '02', title: 'Project Selection', body: 'Finding the right project type — pre-construction, resale, beachfront, hillside — against your use case, hold period, and exit expectations.' },
              { num: '03', title: 'Title & Legal Clarity', body: 'Clean title, clear ownership structure, transparent HOA rules, rental permissions confirmed in writing, and professional closing guidance from an independent Honduran attorney.' },
              { num: '04', title: 'Developer Quality', body: 'Vetting the developer behind a project — track record, completed phases, financial structure, delivery timeline, and alignment between what is promised and what gets built.' },
              { num: '05', title: 'Rental Realism', body: 'Conservative underwriting that separates gross rental revenue from net owner cash flow — after management fees, HOA, maintenance, vacancy, and seasonal slowdowns.' },
              { num: '06', title: 'Exit Strategy', body: 'A future buyer should understand the asset quickly and cleanly. Liquidity depends on location, legal structure, and product quality — all evaluated at entry, not at exit.' },
            ].map((c) => (
              <div key={c.num}>
                <p style={{ fontFamily: 'Georgia, "Times New Roman", serif', fontSize: 28, fontWeight: 700, color: '#789ead', margin: '0 0 10px', lineHeight: 1 }}>{c.num}</p>
                <h3 style={{ fontFamily: 'Georgia, "Times New Roman", serif', fontSize: 17, fontWeight: 400, color: '#ffffff', margin: '0 0 10px' }}>{c.title}</h3>
                <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.55)', lineHeight: 1.75, margin: 0 }}>{c.body}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div style={{ backgroundColor: '#093f4f', padding: 48, textAlign: 'center', color: '#ffffff' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, justifyContent: 'center' }}>
              ...
              <a href="mailto:tomas@tomasfigueroa.com" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '12px 24px', backgroundColor: 'rgba(255,255,255,0.1)', color: '#ffffff', textDecoration: 'none', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.12em', fontFamily: 'Arial, Helvetica, sans-serif', transition: 'background-color 0.2s' }}
                ...
                onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = 'rgba(255,255,255,0.1)')}
              >
                <Mail style={{ width: 16, height: 16 }} /> tomas@tomasfigueroa.com
              </a>
            </div>
